---
alias: [Model Planning, Setup and Assumptions]
created: 2021-03-03T08:06:29 (UTC -05:00)
tags: [Model Planning, Setup and Assumptions | Plum Solutions, Model Planning, Setup and Assumptions]
source: https://plumsolutions.com.au/lessons/model-planning-setup-and-assumptions-fmi/
author: 
---

# Model Planning, Setup and Assumptions | Plum Solutions

> ## Excerpt
> If you have any questions or problems throughout the course, you can get help in the following ways:

---

Model Planning, Setup and Assumptions
If you have any questions or problems throughout the course, you can get help in the following ways:

-   Email us directly at info@plumsolutions.com.au
-   Telephone +612 9818 7250 during business hours AEST (Sydney, Australia)
-   Log into the website, and click on the Live Support button on the bottom right-hand side

_This module forms part of the_ _[](https://www.plumsolutions.com.au/online-courses/financial-modelling-excel-online)_ _[FMI Level 1 – Exam Preparation](https://plumsolutions.com.au/courses/fmi-level-1-exam-preparation/)_ _online course._

The first two videos contain some strategies on how to best set up the file to get started on the exam, and the last one is on general assumptions documentation. Getting your model set up right in the first place is going to save you a HUGE amount of time during the exam, as formatting can be very time-consuming. You need to practice setting up the model with all the correct tabs and formatting within the first 5 minutes as soon as you get into the exam, and that way you can spend the next 3 hours and 55 minutes focussing on building your model!

**Step 1:** Download the Course Materials

-   You can [download all of the Excel files here](https://plumsolutions.com.au/wp-content/uploads/2020/10/Excel-Files.zip) or you can download each file as you proceed through the course.
-   To follow along with the examples in the first two videos, please download a copy of the sample file [“FMI Level 1 AFM Exam Worksheet”](https://plumsolutions.com.au/wp-content/uploads/2020/09/Henderson-Sample-start.xlsx)

**Step 2:** Complete each of the lesson topics  
**Step 3:** Take the quiz

[← Previous Lesson](https://plumsolutions.com.au/lessons/three-way-modelling-fmi/) [Next Lesson →](https://plumsolutions.com.au/lessons/designing-for-scenario-analysis/)

[![468](Model%20Planning,%20Setup%20and%20Assumptions/7-day-post-promo-689.jpg)](https://plumsolutions.com.au/elearning/7-days-financial-modelling/)
