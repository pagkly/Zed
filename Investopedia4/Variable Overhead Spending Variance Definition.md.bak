...
> ## Excerpt
> [[Variable Overhead|Variable overhead]] spending variance is the difference between actual variable overheads and standard variable overheads based on the budgeted costs.
...
A spending variance is the difference between the actual amount of a particular expense and the expected (or budgeted) amount of an expense. To understand what [[Variable Overhead|variable overhead]] spending variance is, it helps to know what a variable overhead is. Variable overhead is a cost associated with running a business that fluctuates with operational activity. As production output increases or decreases, variable overheads move in tandem. Overheads are typically a [[Fixed Cost|fixed cost]], for example, administrative expenses. Variable overheads, on the other hand, are tied to production levels.

[[Variable Overhead|Variable overhead]] spending variance is the difference between actual variable overhead cost, which is based on the costs of indirect materials involved in [[Manufacturing|manufacturing]], and the budgeted costs called the standard variable overhead costs.
...
-   The standard [[Variable Overhead|variable overhead]] rate is typically expressed in terms of machine hours or labor hours.
-   [[Variable Overhead|Variable overhead]] spending variance is favorable if the actual costs of indirect materials are lower than the standard or budgeted variable overheads.
-   [[Variable Overhead|Variable overhead]] spending variance is unfavorable if the actual costs are higher than the budgeted costs.
...
Let's say that actual labor hours used are 140, the standard or budgeted [[Variable Overhead|variable overhead]] rate is $8.40 per direct labor hour and the actual variable overhead rate is $7.30 per direct labor hour. The variable overhead spending variance is calculated as below:

Standard [[Variable Overhead|variable overhead]] Rate $8.40 − Actual Variable Overhead Rate $7.30 =$1.10
...
An [[Unfavorable Variance|unfavorable variance]] may occur if the cost of indirect labor increases, cost controls are ineffective, or there are errors in budgetary planning.

## Limitations

### Fast Fact

[[Variable Overhead|Variable overhead]] spending variance is essentially the difference between the actual cost of variable production overheads versus what they should have cost given the output during a period.
