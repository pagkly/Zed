...
This leads to a dilemma, though. Ultimately the trader must determine when to end the grid, exit the trades, and realize the profits. Otherwise, the price could [[reverse]](https://www.investopedia.com/terms/r/[[Reversal|reversal]].asp) and those profits [[Will|will]] disappear. While losses are controlled by the sell orders, also equally spaced, by the time those orders are reached the position could have gone from profitable to losing money.
...
The problem with the against-the-trend grid is that the [[Risk|risk]] is not controlled. The trader could end up accumulating a larger and larger losing position if the price keeps running in one direction instead of ranging. Ultimately, the trader must set a [[stop loss level]](https://www.investopedia.com/articles/[[Stocks|stocks]]/09/use-stop-loss.asp), as they can't continue to [[Hold|hold]] a losing (let alone make bigger) position indefinitely.
...
-   Determine whether the grid [[Will|will]] be with-the-trend or against-the-trend.
...
Assume the trader opts to use an against-the-trend grid. They also choose 1.1550 as the starting point and a 10 pip interval. They place buy orders at 1.1540, 1.1530, 1.1520, 1.1510, and 1.1500. They place sell orders at 1.1560, 1.1570, 1.1580, 1.1590, and 1.1600. This strategy [[Will|will]] lock in profits as both buy and sell orders are triggered, but it requires a stop loss if the price moves in one direction.

## Example of Grid Trading in the EURUSD

Assume a day trader sees that the [[EURUSD]](https://www.investopedia.com/terms/forex/e/[[EUR|eur]]-usd-euro-us-dollar-currency-pair.asp) is ranging between 1.1400 and 1.1500. The price is currently near 1.1450, so the trader opts to use a 10 pip interval against-the-trend grid to potentially capitalize on the range.

The trader places a sell order at 1.1460, 1.1470, 1.1480, 1.1490, 1.1500, and 1.1510. A stop loss is placed at 1.1530. This assures there is a cap to the [[Risk|risk]]. The risk is 270 pips if all the sell orders are triggered, no grid buy orders are triggered, and the stop loss is reached.

They also place buy orders at 1.1440, 1.1430, 1.1420, 1.1410, 1.1400, and 1.1390. They place a stop loss at 1.1370. The [[Risk|risk]] is 270 pips if all the buy orders are triggered, no grid sell orders are triggered, and the stop loss is reached.

The trader is hoping the price [[Will|will]] move higher and lower, or lower and higher within the range of 1.1510 and 1.1390. Although they are also hoping that the price doesn't move too far outside that range, otherwise they will be forced to exit with a loss in order to control their [[Risk|risk]].
