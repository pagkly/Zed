...
A signature is an endorsement. For example, when an employer signs a [[Payroll|payroll]] [[check]](https://www.investopedia.com/terms/c/check.asp), it authorizes or endorses the transfer of money from the business account to the employee. The act of signing the check is considered an endorsement, which serves as proof of the payer's intent to transfer funds to the [[Payee|payee]].

In a [[financial transaction]](https://www.investopedia.com/articles/financial-theory/09/[[Risk|risk]]-free-rate-[[Return|return]].asp) where one party gives a check to another, the [[Payee|payee]] named on the check must endorse the check before it is cashed. The payee endorses the check by signing it on the back. A signature on the back of the check indicates that the transaction is complete and allows the transfer of money ordered by the check.

If more than one person is [[Listed|listed]] as a [[Payee|payee]] on the check, then the endorsement requirements differ depending on how the names are written. (For example, if the check is written out to John Doe _and_ Jane Doe, both people must sign the check.) If the check is written out to John Doe _or_ Jane Doe, then only one signature is required.
...
In the field of marketing, supporters of products are sometimes called "influencers." Often times influencers [[Leverage|leverage]] [[Social Media|social media]] to market the objects of their support.
