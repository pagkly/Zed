...
> ## Excerpt
> A [[Non-Sampling Error|non-sampling error]] is an error that results during data collection, causing the data to differ from the true values.
...
A [[Non-Sampling Error|non-sampling error]] is a statistical term that refers to an error that results during data collection, causing the data to differ from the true values. A non-sampling error differs from a [[sampling error.]](https://www.investopedia.com/terms/s/samplingerror.asp) A sampling error is limited to any differences between sample values and universe values that arise because the sample size was limited. (The entire universe cannot be sampled in a survey or a census.)

### Key Takeaways

-   A [[Non-Sampling Error|non-sampling error]] is a term used in statistics that refers to an error that occurs during data collection, causing the data to differ from the true values. 
-   A [[Non-Sampling Error|non-sampling error]] refers to either random or systematic errors, and these errors can be challenging to spot in a survey, sample, or census. 
...
Random errors are believed to offset each other and therefore, most often, are of little concern. Systematic errors, on the other hand, affect the entire sample and therefore present a more significant issue. Random errors, generally, [[Will|will]] not result in scrapping a sample or a census, whereas a systematic error will most likely render the data collected unusable.
