...
Using Iroha, a business can create and manage simple digital assets like any standard [[cryptocurrenc]](https://www.investopedia.com/terms/c/[[Cryptocurrency|cryptocurrency]].asp)[[Y|y]], or complex ones like indivisible rights, certificate authenticity, and patents.
...
Iroha can also be used to create digital avatars of real-world assets that can be transacted with zero or low transaction fees. For instance, the current owner of a [[Vintage|vintage]] car can create a digital asset that represents the vintage car on the blockchain, and then link its ownership to himself. To transfer ownership, he can then create an [[Offer|offer]] using multi-signature transaction which includes the cost of transfer in a particular currency. The interested [[Counterparty|counterparty]] can accept the offer on the blockchain and complete the transaction by transferring the currency to the present owner, and receive the ownership of the car in [[Return|return]].
...
Iroha differs from other popular blockchain networks, like [[Bitcoin]](https://www.investopedia.com/terms/b/bitcoin.asp) and [[Ethereum]](https://www.investopedia.com/terms/e/[[Ethereum|ethereum]].asp), as the latter operate as permissionless ledgers which allow anyone to join and [[Grant|grant]] access to everything on the network. Iroha’s operations are permissioned, that is, only participants with suitable access are allowed to join, interact and contribute to the blockchain system. Data querying is also restricted on Iroha, as not everyone is allowed to read and verify the data on the blockchain. Unlike Bitcoin or Ethereum, Iroha does not have a native [[Cryptocurrency|cryptocurrency]], but it can be created by an eligible participant as required for their own enterprise use.

Iroha also claims to [[Offer|offer]] advantages over [[Ethereum]]’s smart contracts. While smart contracts on Ethereum may require writing cumbersome code, the same can be achieved quickly and simply by using the built-in commands in Iroha to complete common tasks more quickly and with lower complexity and lower [[Risk|risk]].
