...
> ## Excerpt
> The dividend growth rate is the annualized percentage rate of growth of a particular [[Stock|stock]]'s dividend over time.
...
In the above example, the [[Growth Rates|growth rates]] are:
...
The average of these four annual [[Growth Rates|growth rates]] is 3.56%. To confirm this is correct, use the following calculation:

> $1 x (1 + 3.56%) 4 = $1.15

## Example: Dividend Growth and [[Stock]] Valuation

To value a company’s [[Stock|stock]], an individual can use the dividend discount model (DDM). The dividend discount model is based on the idea that a stock is worth the sum of its future payments to shareholders, discounted back to the present day.
...
In the above example, if we assume next year's dividend [[Will|will]] be $1.18 and the [[Cost of Equity|cost of equity]] capital is 8%, the [[Stock|stock]]'s current price per share calculates as follows:
