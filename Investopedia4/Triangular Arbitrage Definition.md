---
alias: [Triangular Arbitrage]
created: 2021-03-03T00:09:09 (UTC +11:00)
tags: [Triangular Arbitrage Definition, Triangular Arbitrage Definition]
...
Automated trading platforms have streamlined the way trades are executed, as an algorithm is created in which a trade is automatically conducted once certain criteria is met. Automated trading platforms allow a trader to set rules for entering and exiting a trade, and the computer [[Will|will]] automatically conduct the trade according to the rules. While there are many benefits to automated trading, such as the ability to test a set of rules on historical data before risking capital, the ability to engage in triangular arbitrage is only feasible using an automated trading platform. Since the market is essentially a self-correcting entity, trades happen at such a rapid pace that an arbitrage opportunity vanishes seconds after it appears. An automated trading platform can be set to identify an opportunity and act on it before it disappears.

That said, the speed of algorithmic trading platforms and markets can also work against traders. For example, there may be an execution [[Risk|risk]] in which traders are unable to a lock in a profitable price before it moves past them in seconds.
...
From these transactions, you would receive an arbitrage profit of $1,373 (assuming no [[Transaction Costs|transaction costs]] or taxes).
