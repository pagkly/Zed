...
The Durbin Watson (DW) statistic is a test for [[autocorrelation]](https://www.investopedia.com/terms/a/autocorrelation.asp) in the residuals from a statistical [[regression analysis]](https://www.investopedia.com/terms/r/regression.asp). The Durbin-Watson statistic [[Will|will]] always have a value between 0 and 4. A value of 2.0 means that there is no autocorrelation detected in the sample. Values from 0 to less than 2 indicate positive autocorrelation and values from from 2 to 4 indicate negative autocorrelation.

A [[Stock|stock]] price displaying positive autocorrelation would indicate that the price yesterday has a positive [[Correlation|correlation]] on the price today—so if the stock fell yesterday, it is also likely that it falls today. A [[Security|security]] that has a negative autocorrelation, on the other hand, has a negative influence on itself over time—so that if it fell yesterday, there is a greater likelihood it [[Will|will]] rise today.
...
-   Autocorrelation can be useful in [[Technical Analysis|technical analysis]], which is most concerned with the trends of [[Security|security]] prices using charting techniques in lieu of a company's financial health or management.

## The Basics of the Durbin Watson Statistic

Autocorrelation, also known as [[serial correlation]](https://www.investopedia.com/terms/s/serial-[[Correlation|correlation]].asp), can be a significant problem in analyzing historical data if one does not know to look out for it. For instance, since [[Stock|stock]] prices tend not to change too radically from one day to another, the prices from one day to the next could potentially be highly correlated, even though there is little useful information in this observation. In order to avoid autocorrelation issues, the easiest solution in [[Finance|finance]] is to simply convert a series of historical prices into a series of percentage-price changes from day to day.

Autocorrelation can be useful for [technical analysis](https://www.investopedia.com/terms/t/technicalanalysis.asp), which is most concerned with the trends of, and relationships between, [[Security|security]] prices using charting techniques in lieu of a company's financial health or management. Technical analysts can use autocorrelation to see how much of an impact past prices for a security have on its future price.

The Durbin Watson statistic is named after statisticians James Durbin and Geoffrey Watson.

Autocorrelation can show if there is a [[Momentum|momentum]] factor associated with a [[Stock|stock]]. For example, if you know that a stock historically has a high positive autocorrelation value and you witnessed the stock making solid gains over the past several days, then you might reasonably expect the movements over the upcoming several days (the leading time series) to match those of the lagging time series and to move upward.
...
Assume the following (x,[[Y|y]]) data points:
...
This first step in calculating the Durbin Watson statistic is to calculate the expected "[[Y|y]]" values using the [[Line Of Best Fit|line of best fit]] equation. For this data set, the expected "y" values are:
...
Next, the differences of the actual "[[Y|y]]" values versus the expected "y" values, the errors, are calculated:
