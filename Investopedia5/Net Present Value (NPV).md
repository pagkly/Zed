...
NPV\=∑t\=1nRt(1+i)twhere:Rt\=Net cash inflow-outflows during a single period ti\=Discount rate or return that could be earned inalternative investmentst\=Number of timer periods\\begin{aligned} &NPV = \\sum\_{t = 1}^n \\frac { R\_t }{ (1 + i)^t } \\\\ &\\textbf{where:} \\\\ &R\_t=\\text{Net cash inflow-outflows during a single period }t \\\\ &i=\\text{Discount rate or return that could be earned in} \\\\ &\\text{alternative investments} \\\\ &t=\\text{Number of timer periods} \\\\ \\end{aligned}
...
[Internal rate of return](https://www.investopedia.com/terms/i/irr.asp) (IRR) is very similar to NPV except that the discount rate is the rate that reduces the NPV of an investment to zero. This method is used to compare projects with different lifespans or amount of required capital.
