...
> ## Excerpt
> A derivative that gives the holder the right, but not the obligation, to buy or sell a security at a certain price before expiration.
...
Trading and finding information on warrants can be difficult and time-consuming as most warrants are not listed on major exchanges, and data on warrant issues is not readily available for free. When a warrant is listed on an exchange, its ticker symbol will often be the symbol of the company's common stock with a W added to the end. For example, Abeona Therapeutics Inc's ([ABEO](https://www.investopedia.com/markets/quote?tvwidgetsymbol=abeo)) warrants were listed on Nasdaq under the symbol ABEOW.1 In other cases, a Z will be added, or a letter denoting the specific issue (A, B, C…).
